...
app.post('/db/upsert', async function (req, res) {
  try {
    for (const [key, value] of Object.entries(req.body)) {
      await db.any('INSERT INTO test_table (key, value) VALUES($1, $2) ON CONFLICT (key) DO UPDATE SET value = $2', [key, value])
    }
    res.send("Added data to database");
  } catch (e) {
    res.send(`Unable to insert item into database: ${e}`);
  }
});
